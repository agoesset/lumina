<div x-data="{ modal: @entangle('showModal') }">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Data Fasilitas</h2>
            <p class="text-slate-500">Kelola data fasilitas ibadah</p>
        </div>
            ...
            <div class="p-4 hover:bg-slate-50 transition-colors">
                <div class="flex items-start justify-between mb-3">
                    ...
                    <div class="flex items-center gap-2 shrink-0 ml-2">
                        <button wire:click="edit({{ $f->id }})" class="p-2 rounded-xl bg-primary-50 text-primary-600 hover:bg-primary-100 hover:scale-110 transition-all duration-150" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </button>
                        <button wire:click="delete({{ $f->id }})" wire:confirm="Yakin hapus {{ $f->nama }}?" class="p-2 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 hover:scale-110 transition-all duration-150" title="Hapus">
                ...
                <div class="grid grid-cols-2 gap-2 text-sm">
                    <div>
                        <span class="text-slate-400 text-xs">Jumlah</span>
                        <p class="font-medium text-slate-700">{{ $f->jumlah }}</p>
                    </div>
                    <div>
                        <span class="text-slate-400 text-xs">Kondisi</span>
                        @php
                        ...
                        @endphp
                        <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium {{ $colors[$f->kondisi] }}">{{ $labels[$f->kondisi] }}</span>
                    </div>
                </div>
            </div>
            @empty
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gradient-to-r from-primary-50 to-primary-50/50">
                        ...
                        <th class="px-5 py-3.5 text-left text-xs font-semibold text-primary-700 uppercase tracking-wider"><span class="inline-flex items-center gap-2"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> Jumlah</span></th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        ...
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 font-semibold text-slate-700 text-sm">{{ $f->jumlah }}</span>
                        </td>
                        <td class="px-5 py-4">
                            @php
                                $colors = ['baik' => 'bg-emerald-100 text-emerald-700', 'perlu-perawatan' => 'bg-orange-100 text-orange-700', 'rusak' => 'bg-red-100 text-red-700', 'perlu-dicuci' => 'bg-yellow-100 text-yellow-700'];
                                $labels = ['baik' => 'Baik', 'perlu-perawatan' => 'Perlu Perawatan', 'rusak' => 'Rusak', 'perlu-dicuci' => 'Perlu Dicuci'];
                                $icons = ['baik' => 'check', 'perlu-perawatan' => 'exclamation', 'rusak' => 'x', 'perlu-dicuci' => 'refresh'];
                            @endphp
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $colors[$f->kondisi] }}">
                                @if($icons[$f->kondisi] === 'check')
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                @elseif($icons[$f->kondisi] === 'exclamation')
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button wire:click="edit({{ $f->id }})" class="p-2 rounded-xl bg-primary-50 text-primary-600 hover:bg-primary-100 hover:scale-110 active:scale-95 transition-all duration-150 shadow-sm" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </button>
                                <button wire:click="delete({{ $f->id }})" wire:confirm="Yakin hapus {{ $f->nama }}?" class="p-2 rounded-xl bg-red-50 text-red-500 hover:bg-red-100 hover:scale-110 active:scale-95 transition-all duration-150 shadow-sm" title="Hapus">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                </button>
                            </div>
                        </td>
    ...
    <div x-show="modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" x-cloak>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div class="p-5 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-slate-800">{{ $editId ? 'Edit Fasilitas' : 'Tambah Fasilitas' }}</h3>
            ...
            <form wire:submit="save" class="p-5 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Nama Fasilitas</label>
                    <input type="text" wire:model="nama" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 text-sm" required>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Kategori</label>
                        <input type="text" wire:model="kategori" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 text-sm" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Jumlah</label>
                        <input type="number" wire:model.number="jumlah" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 text-sm" min="1" required>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" wire:model="lokasi" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 text-sm" required>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Tanggal Pembaruan</label>
                    <input type="date" wire:model="tanggal_pembaruan" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 text-sm" required>
                </div>
